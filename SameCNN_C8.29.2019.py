...
@author: Marcus Wong
"""
## **This script is used to take an MRI image, performs an aliased + blur mask, 
## and then reconstructs the image using a small same convolutional neural network (CNN).
...
# we check that the images, every 25th one is different

#imshow(mri_images[1], cmap = "gray")
#imshow(mri_images[26], cmap="gray")
#imshow(mri_images[1]-mri_images[26], cmap = "gray")

# Every 25th image is the same anatomy but with in a different slice, so they
# should be similar.
...
# We check the shuffled data
#imshow(mri_data[1], cmap = "gray")
#imshow(mri_data[56], cmap = "gray")

# In[2]: Here we are creating a function that has a dataset as an imput and in the loop
...
def fourier_operation(user_Images): 
    ...
    for j in range(0,user_Images.shape[0]): # for loop going from 0 to number of images in dataset
        ...
        # The rest is hardcoded to have every other column be equal to 1
        matrix_of_ones[:, 0] = 1 
        matrix_of_ones[:, 2] = 1
        matrix_of_ones[:, 4] = 1
        matrix_of_ones[:, 6] = 1
...
original_image_MRI = mri_images[903,:,:] # Calls the 903rd image of the original dataset and assigns it to a variable
blurred_version_image_MRI = blurred_MRI_Image[903,:,:] # Calls the 903rd image of the new datset after the mask is applied and assigns it to a variable
...
rmse1 = np.sqrt(err) 
print("The RMSE of the new image vs the original image is ", rmse1)
# In[2]: This section we are seperating our data into training and testing sets
...
#X_train = X_train/(X_train.max()) # Normalizes the x training set
...
#X_test = X_test/(X_test.max()) # Normalizes the x testing set
...
#Y_train= Y_train/(Y_train.max()) # Normalizes the y training set
...
#Y_test = Y_test/(Y_test.max()) # Normalizes the y testing set
...
# In[7]: This evaluates our model on the training set and prints out the loss value

preds = sameCNN.evaluate(x = X_train, y = Y_train) # Evaluate CNN Model on training set

#Prints loss value
print()
print ("Loss = " + str(preds))
# In[8]: This evaluates our test set based on the model trained by our training set and prints out the loss value
...
# In[9]: We are plotting the loss value after every epoch. Y is the loss value in log scale and X is the number of epochs
...
# In[10]: Here we are Displaying all three images side by side. The Original, aliased & blurred, and reconstructed image
...
ax1.imshow(mri_images[903], cmap='gray') # Displaying the 903rd original image
plt.axis('off')
ax2 = fig.add_subplot(1,3,2)
ax2.imshow(X_test[103, ..., 0], cmap = "gray") # Diplaying the 103rd image from the X_test set
plt.axis('off')
ax3 = fig.add_subplot(1,3,3)
ax3.imshow(pred[103, ..., 0], cmap='gray') # Displaying our reconstructed image of the 103rd image
plt.axis('off')  
ax1.title.set_text('Original Image')
ax2.title.set_text('Aliased + Blur Image')
...
err = np.sum((mri_images[903].astype("float") - pred[103, ..., 0].astype("float")) ** 2) #
err /= float(mri_images.shape[1] * mri_images.shape[1]) # Divide error by number of pixels
rmse2 = np.sqrt(err) # Take square root or error 
print("The RMSE of the new image vs the original image is ", rmse2)

# In[11]:
# Produces a nice summary block of our model
plot_model(sameCNN, to_file='CNN A.png')
SVG(model_to_dot(sameCNN).create(prog='dot', format='svg'))
